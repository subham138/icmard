		...
		<div class="content-wrapper">
			<div class="card">
				<div class="card-body">
					...
					<div class="row">
						<div class="col-sm-12">
							<form action="<?= base_url() ?>index.php/stock/edit_out" method="post" id="form">
								...
								<div class="form-group row">
									...
									<div class="col-sm-4">
										<select name="item" class="form-control select2" id="item" required>
											...
											?>
												<option value="<?php echo $item->id; ?>" <?= $selected ?>><?php echo $item->item_name; ?></option>
											<?php
											}
											?>
										</select>
									...
									<div class="col-sm-4">
										<input type="text" class="form-control" placeholder="" name="stock" id="stock" value="<?= $cust->stock ?>" readonly required>
									</div>
								...
								<div class="form-group row">
									...
									<div class="col-sm-4">
										<select name="vendor" class="form-control select2" id="vendor">
											...
											?>
												<option value="<?= $vendor->uin; ?>" <?= $selected ?>>
													<?= $vendor->cust_name; ?>
								...
								<div class="form-group row">
									...
									<div class="col-sm-4">
										<select name="recived_by" class="form-control select2" id="recived_by" required>
											...
											$selected = '';
											foreach ($tenantdtls as $vendor) {
												$selected = $cust->received_by == $vendor->uin ? 'selected' : '';
											?>
												<option value="<?= $vendor->uin; ?>" <?= $selected ?>>
													<?= $vendor->cust_name; ?>
													<?= $vendor->cust_type == 'C' ? '<small>(Customer)</small>' : ($vendor->cust_type == 'V' ? '(Vendor)' : '(Tenant)') ?>
